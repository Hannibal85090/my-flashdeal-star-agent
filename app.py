...
from services.voice_gateway import VoiceGatewayService

# إعداد واجهة الويب وعناصر النيون المتوافقة مع أبعاد الهاتف الذكي
...
# 2. بوابة الاستماع الصوتي الذكي (Speechmatics)
st.markdown("<div class='glass-card'>", unsafe_allowed_html=True)
st.subheader("🎙️ Agentic Voice Gateway")
audio_input = st.file_uploader("Upload or Record Voice Command (Audio File)", type=["wav", "mp3", "m4a"])

voice_command = ""
if audio_input:
    st.audio(audio_input, format="audio/wav")
    with st.spinner("Routing audio to Speechmatics high-fidelity engine..."):
        voice_service = VoiceGatewayService()
        voice_command = voice_service.transcribe_command(audio_input)
        st.success(f"Transcribed Text: '{voice_command}'")
st.markdown("</div>", unsafe_allowed_html=True)

# 3. سياق السوق وبوابة Bright Data
st.markdown("<div class='glass-card'>", unsafe_allowed_html=True)
...
if st.button("Fetch Market Rates via Bright Data API"):
    with st.spinner("Scraping and structuring global web contexts live..."):
...
# 4. محرك الإطلاق النهائي للعملية المالية
